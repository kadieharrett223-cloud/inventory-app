...
const navItems = [
  ...
  { href: "/logistics", label: "Logistics", icon: ShoppingCart },
...
export function AppShell({ children }: AppShellProps) {
  ...
  return (
    <div className="grid min-h-screen grid-cols-1 bg-[#f2f4f8] text-[#182233] lg:grid-cols-[236px_1fr]">
      <aside className="relative border-r border-[#1e2633] bg-[linear-gradient(180deg,#0d131c_0%,#0a1017_55%,#090e15_100%)] text-white">
        <div className="relative flex h-full flex-col px-3.5 py-4.5">
          <Link href="/" className="px-1 py-0.5">
            <p className="text-[31px] font-black leading-none tracking-tight text-white">OLYMPIC</p>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`relative flex items-center gap-2.5 rounded-xl px-2.5 py-2.5 text-[14px] font-semibold transition ${
                    active
          ...
          <div className="mt-auto space-y-3">
            <div className="rounded-lg border border-white/10 bg-white/[0.02] p-3.5">
              ...
              <ul className="mt-2.5 space-y-2 text-[12px] text-white/84">
            ...
            <div className="flex items-center justify-between rounded-lg border border-white/10 bg-white/[0.02] px-2.5 py-2.5">
              <div className="flex items-center gap-2">
                <div className="flex h-6 w-6 items-center justify-center rounded-full bg-white/15 text-[10px] font-bold">KH</div>
                <div>
                  <p className="text-[12px] font-semibold text-white">Kadie Harrett</p>
