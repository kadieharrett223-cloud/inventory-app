...
export function AppShell({ children }: AppShellProps) {
  ...
  return (
    <div className="grid min-h-screen grid-cols-1 bg-[#f2f4f8] text-[#182233] lg:grid-cols-[224px_1fr]">
      <aside className="relative border-r border-[#1e2633] bg-[linear-gradient(180deg,#0d131c_0%,#0a1017_55%,#090e15_100%)] text-white">
        <div className="relative flex h-full flex-col px-3 py-4">
          <Link href="/" className="px-1.5 py-1">
            <p className="text-[28px] font-black leading-none tracking-tight text-white">OLYMPIC</p>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`relative flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-[13px] font-semibold transition ${
                    active
      ...
      <main className="overflow-x-hidden px-5 pb-5 pt-7 lg:px-6">{children}</main>
    </div>
  );
}
